...
describe("recoveryDisabledMessage", () => {
  it("presents actionable core compatibility diagnostics without parsing message text", () => {
    const error = new WorkbenchJournalLoadError(
      "Workbench run journal version 2 is not supported. Upgrade OpenSpec Workbench to recover runs.",
      {
    ...
    expect(recoveryDisabledMessage(error)).toContain("Upgrade OpenSpec Workbench to recover runs");
  });
});