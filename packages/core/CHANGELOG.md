...
- f0764af: The local LLM (agent `local-llm`) can be told where it is, which model to
...
- a8a08fc: DeepSeek runs are refused before they start on a Node that cannot run
...
- 478ae8f: The chain's spending ceiling carries its unit. `budget.maxCost` takes a
  ceiling per unit of account - `{ "credits": 500 }` - checked against what
  was reported in that same unit, with nothing converted and no total mixing
  units. A ceiling in a unit no stage's agent is billed in is reported before
  the run, beside the other ceilings that cannot act. The run budget field in
  the settings says it is in dollars, and says where a ceiling in another
  unit is set.
...
- 3c79199: A standing whose repository has no remote on a GitHub host gh knows says so, rather than that gh is not signed in.
...
- 8a71bd3: A status write never stops a run.
...
- 82d5021: A chain can archive a change in any repository, not only in one whose rules say to tick tasks.
...
- 7fc5535: A suggestion naming a dozen changes is now readable.
  
  `maxSets` capped how many groupings were named and nothing capped the width of one, so twelve changes that could all run together produced a subject joining all twelve names with "and". Past a few, the rest are counted — "alpha, beta, delta and 9 more can run at the same time". The commands still name every change, because those are the work; it was the headline that had to be read rather than scrolled past.
