...
export function ChangesList({ changes, onSelect, states, sources, onRefresh, refreshing = false, refreshError, footnote }: ChangesListProps) {
          ...
          {rows.map(({ item: change, key, style }) => {
            ...
            return (
              <li key={key} style={style} className={withLines ? "openspec-row--tall" : undefined}>
                <button
                  type="button"
                  className="openspec-row openspec-change-columns"
