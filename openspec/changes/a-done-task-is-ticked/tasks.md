...
- [ ] 1.1 `commandInstruction("implement")` in `agents/shared.ts` tells
  ...
  it could not do unticked and say why.
- [ ] 1.2 `commandInstruction("verify")` tells the agent to tick an
...
- [ ] 1.3 Neither instruction depends on the project's configured rules:
  the rules section is still added where a project has one, and says
  nothing the instruction relies on.

## 2. The chain

- [ ] 2.1 `harness-chain-runner.ts` counts ticked tasks before the apply
  stage and again after it.
- [ ] 2.2 When the apply stage completed, its checkpoint delta is not
...
- [ ] 2.3 The archive refusal for unticked tasks names them through
  `unfinishedTaskTexts`, the function the return from verification
  already uses.

## 3. Documentation

- [ ] 3.1 `HARNESS.md`'s stage table says that `apply` ticks each task it
  finished, and that `verify` ticks what it confirmed and unticks what
  does not hold.

## 4. Tests

- [ ] 4.1 The `implement` instruction contains the tick rule; a test pins
  the phrase that carries it.
- [ ] 4.2 The `verify` instruction contains the tick rule, the rule about
  effects that are not files, and the exclusion of human-only and
  delegated tasks; a test pins each.
- [ ] 4.3 A chain whose apply run changes a file and ticks nothing emits
  the report and still reaches verification.
- [ ] 4.4 A chain whose apply run changes no file emits no such report.
- [ ] 4.5 A chain whose apply run ticks a task emits no such report.
- [ ] 4.6 The archive refusal names each unticked task.
