...
- [x] 1.1 `packages/extension/src/extension.ts`: `ExtensionTestApi` gains
...
- [x] 2.2 Same: assert the chat-opening command was invoked with the
  built prompt. Observe the command; assert nothing about a window.
- [x] 2.3 Same: a stage **not** set to the chat agent still runs through
  the ordinary agent path. The dispatch must not become the default by
  accident.
- [x] 2.4 Use the real Extension Development Host the suite already runs —
...
- [x] 3.1 `harness-stage-dispatch` task 6.6: reduce it to what needs eyes —
  that the chat window opens with the prompt in it — and point at the
  integration case for the event contract.
- [x] 3.2 Do **not** mark 6.6 done as part of this change. Narrowing what
  a person must check is not the same as having checked it.

## 4. Verification

- [x] 4.1 `openspec change validate --strict dispatch-to-chat-integration-coverage`.
- [x] 4.2 `npm run typecheck`, `npm run lint`, `npm run test` — green
  across all five workspaces.
- [x] 4.3 `npm run test:integration --workspace openspec-ui-vscode` — the
  new case passes in the real host.
- [x] 4.4 Confirm the new case **fails** when the dispatch is disabled. A
...
- [x] 4.5 `git diff packages/extension/src/webview/ai-panel.ts` contains
  no behaviour change. This change adds coverage; it does not alter what
  is covered.
- [x] 4.6 No changeset — test-only, matching
