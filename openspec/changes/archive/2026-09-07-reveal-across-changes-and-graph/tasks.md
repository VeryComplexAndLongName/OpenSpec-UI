...
- [x] 1.1 `ChangesTreeProvider.getParent` and `ArchiveTreeProvider.getParent`.
...
- [x] 1.2 `ChangeGraphTreeProvider.getParent`. A row's identity is the
...
- [x] 1.3 Register the graph with `createTreeView` rather than
...
- [x] 1.4 Keep the graph read-only. Revealing is not acting: no mutating
  command gains a graph entry, and the reasoning in
  `change-graph-in-vscode` stands.

## 2. Reveal in Change Graph

- [x] 2.1 `openspec-ui.revealInChangeGraph`, offered on a change in both
  the Changes and Archive trees, matching how `showChangeAncestry` is
  offered on both.
- [x] 2.2 Find every row for the change, not the first. A row exists per
...
- [x] 2.3 Reveal each, select the first, and report the count when it is
  more than one — "shown in 3 places" rather than silently landing on
  one.
- [x] 2.4 When the change has no row, say that it states no relation and
...
- [x] 3.1 `openspec-ui.revealInChanges`, offered on a graph row.
- [x] 3.2 Route by the row's own `archived` flag: an archived change is
...
- [x] 3.3 A row whose change has since been archived or deleted — the
...
- [x] 4.1 `openspec-ui.followSelectionInChangeGraph`, boolean, default
  ...
  nothing to reveal.
- [x] 4.2 When on, subscribe to the Changes and Archive views'
...
- [x] 4.3 When on and the change has no row, do nothing quietly. The
  explicit command explains; the automatic one must not interrupt.
- [x] 4.4 Unsubscribe when the setting is turned off, without requiring a
  reload.

## 5. Tests

- [x] 5.1 `getParent` on each provider: a change resolves to `undefined`,
  an artifact to its change, a graph row to the row above it on its path.
- [x] 5.2 A change with one row reveals it; a change with three rows
  reveals three and reports three; a change with none reports that it
  states no relation.
- [x] 5.3 A graph row for an archived change reveals into Archive, an
  active one into Changes.
- [x] 5.4 With the setting off, a selection change reveals nothing —
  asserted directly, since this is the default every user gets.

## 6. Verification

- [x] 6.1 `openspec change validate --strict reveal-across-changes-and-graph`.
...
- [x] 6.3 Version bump via `npx changeset`: two commands and a setting.
- [x] 6.4 Document both commands and the setting in the extension README,
  beside the Change Graph entry this change builds on.
- [x] 6.5 **Human-only**: with the setting off, use both commands on a
