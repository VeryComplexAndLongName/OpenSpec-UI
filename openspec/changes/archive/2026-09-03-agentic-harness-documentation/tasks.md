...
- [x] 1.1 New `HARNESS.md` at the repository root: what the harness is in
...
- [x] 1.2 Same file: the two configuration files —
  ...
  allowlist) and why.
- [x] 1.3 Same file: every key with its accepted values — `stepAgents`,
  `autonomyLevel`, `reviewGate`, `checkpoints`, `budget`,
  `gitStageAllowlist`. Read them out of `harness-config.ts`, not from
  memory.
- [x] 1.4 Same file: a worked `agent-harness.json` and a worked
  `harness.json`, both of which must actually load. Asserted by task 6.3
  rather than trusted.
- [x] 1.5 Same file: where each setting is edited in the standalone UI
  and in VS Code, with the screenshots from section 3.
- [x] 1.6 Same file: mechanical checks — the six names, the check
...
- [x] 2.1 `HARNESS.md`: one table with a row per agent id. Columns: id,
...
- [x] 2.2 Fill the effort and budget columns from
...
- [x] 2.3 The "run against the real binary here" column repeats what
  ...
  the run, not after.
- [x] 2.4 State what changes when an `-acp` id is chosen instead of its
  ...
  gate.
- [x] 2.5 **`copilot-cli-acp` and `claude-cli-acp` currently refuse
...
- [x] 2A.1 `HARNESS.md`: a chain does not always start at `propose`.
...
- [x] 2A.2 Same file: when progress cannot be determined the chain enters
...
- [x] 2A.3 Same file: **a chain runs forward only.** There is no control
...
- [x] 2A.4 Same file: what a checkpoint offers. Under `semi-autonomous` a
...
- [x] 2A.5 Same file: **only one mutating run at a time, for the whole
  ...
  wrong one.
- [x] 2A.6 Same file: what *is* concurrent — a non-mutating run (status,
...
- [x] 2B.1 `HARNESS.md`: a section on the `git` stage — what it does
  (push the change's branch, open a pull request, merge it), and that it
  is the last stage, after `archive`.
- [x] 2B.2 Same section: it runs **only** when the resolved
  ...
  default is safe.
- [x] 2B.3 Same section: the per-change `gitStageAllowlist`
  ...
  load.
- [x] 2B.4 Same section: **the merge waits for the pull request's checks
  ...
  cite it.
- [x] 2B.5 Same section: the prerequisites — `gh` on `PATH` and already
  ...
  push.
- [x] 2B.6 Same section: every push, pull-request creation and merge is
  written to the audit log, blocked attempts included. Say where the log
  is, so the sentence is actionable.
- [x] 2B.7 Same section: state plainly that **no one has run this stage
...
- [x] 3.1 New Playwright spec under `packages/server/e2e/` whose product
  ...
  Done: `packages/server/e2e/harness-screenshots.spec.ts`, run via
  `npm run test:browser -- harness-screenshots.spec.ts` from
  `packages/server` — produces `harness-settings.png`,
  `harness-change-override.png`, `harness-checkpoint.png`. Verified
  passing locally (1 passed).
- [x] 3.2 Document the one command that regenerates them, in `HARNESS.md`
  and in the spec's own header comment.
- [x] 3.3 Do **not** add an image-diff gate to CI. Font and platform
...
- [x] 3.5 Replace `docs/images/standalone/harness-settings.png`. The
  ...
  target and the mechanical `archive` row. Done: regenerated by
  `harness-screenshots.spec.ts` (task 3.1), now shows all three.

## 4. `LIMITS.md`

- [x] 4.1 New `LIMITS.md` at the repository root: the two independent
...
- [x] 4.2 Same file: why there is no single `budget: number`. GitHub
...
- [x] 4.3 Same file: which agent honours which field — `claude-cli`
...
- [x] 4.4 Same file: **what does not exist.** There is no wall-clock or
...
- [x] 4.5 Same file: where the numbers a ceiling is compared against come
  from — the audit log's recorded `usage` — and therefore that a ceiling
  can only count what an agent reported.
- [x] 4.6 Same file: one sentence pointing at
...
- [x] 5.1 `README.md`: a harness section linking both documents. The word
  "harness" does not currently appear in that file at all.
- [x] 5.2 `packages/server/README.md` and
  `packages/extension/README.md`: link from each one's own harness
  material.
- [x] 5.3 `AGENTS.md` and `CLAUDE.md`: link, as pointers only. Both files
  say "pointers, not duplicates" and neither may grow a copy of the
  settings tables.
- [x] 5.4 `docs/adr/README.md`: leave alone. ADRs record decisions, and a
  user-facing guide is not one.

## 6. Verification

- [x] 6.1 `openspec change validate --strict agentic-harness-documentation`.
  Done: "Change "agentic-harness-documentation" is valid".
- [x] 6.2 Every agent id in `AGENT_REGISTRY`, plus `vscode-chat`, appears
  ...
  to land with no row. Done:
  `packages/core/src/harness-config.test.ts`'s `describe("HARNESS.md")`.
- [x] 6.3 Both worked examples in `HARNESS.md` load through
  `resolveHarnessConfig` without error. Assert it; an example
  configuration that does not parse is worse than none. Done: same
  `describe("HARNESS.md")` block, "loads both worked harness.json
  examples through resolveHarnessConfig without error".
- [x] 6.4 `npm run lint:english` — both documents are English only. Done:
  "English policy check passed."
- [x] 6.5 No `packages/*/src` non-test source file changes, apart from the
  new Playwright spec. This change documents behavior; it does not alter
  it. Done: this change's own edits touch only
  `packages/core/src/harness-config.test.ts` (a test file) and
  `packages/server/e2e/harness-screenshots.spec.ts` (under `e2e/`, not
  `src/`) — no non-test `packages/*/src` file was changed by this change.
  (Unrelated pre-existing staged edits to `harness-config.ts`/
  `harness-step-agent.ts`/`default-runners.test.ts` belong to two other,
  separately tracked changes already in progress, not to this one.)
...
- [x] 6.7 **Human-only, rechecked and confirmed 2026-09-02**:
  ...
  **Standalone half done 2026-09-02**, against the images task 3.1's spec
  produces from the running application — which is the surface, rendered.
  Checked line by line against the document's "Where each setting is
  edited" table:

  - agent, effort and budget controls appear per stage in both the
    "Global default" and "Per-change override" sections, and the effort
    and budget fields appear only where the stage's agent accepts them —
    `propose` on Claude CLI shows "propose effort" and "propose max cost
    (USD)"; the stages left at `(none)` show neither.
  - no model control anywhere, as the document says.
  - `autonomyLevel` is a select; `reviewGate` is shown as text with the
    note that only a per-change override can relax it.
  - `archive` is a row reading "runs mechanically — no agent", with no
    picker.
  - `git` **does** carry an agent picker, exactly as the document's own
    `stepAgents.git` warning says it does. The document and the screen
    agree, including about the defect.

  The checkpoint image confirms the other claims: it reads "Paused at
  checkpoint", "Continue to **verify** with claude-cli?", and offers
  **Continue** and **Cancel** and nothing else — no way back to an
  earlier stage, per 2A.3 and 2A.4. Its event list starts at `implement`
