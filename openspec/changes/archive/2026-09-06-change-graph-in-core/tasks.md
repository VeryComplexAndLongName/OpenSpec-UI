...
- [x] 1.1 `packages/core/src/change-graph.ts`: read every
...
- [x] 1.2 Port the narrow parser as it stands, including the rule that a
...
- [x] 1.3 Port the reporting of a key present in a shape the parser does
  not accept. An unreadable edge must not read as "no edge stated".
- [x] 1.4 Export the types the extension will need, so
  `change-graph-in-vscode` adds no parsing of its own.

## 2. The blocking relation

- [x] 2.1 `blocked_by: [<change-id>, ...]` — changes that must land
  before this one can start. Accepts the same three shapes as the other
  two keys.
- [x] 2.2 Resolution and cycle detection cover it, on the same terms.
- [x] 2.3 An unmet blocker — a named change still active — is **not** a
  failure. Expose it as state on the node so a renderer can report it.
- [x] 2.4 Keep it out of the historical tree's parent edges. `follows`
  ...
  stated.
- [x] 2.5 Document all three keys in `openspec/README.md`, replacing the
  two-key description, and say plainly which resolves and which does not.

## 3. The gate as a test

- [x] 3.1 `packages/core/src/change-graph.test.ts`: unit tests over
...
- [x] 3.2 One test that reads **this repository's own** `openspec/changes`
...
- [x] 3.3 That test does real filesystem work over a directory that grows
...
- [x] 3.4 Delete `scripts/check-change-graph.mjs` and its test, and
  remove `lint:change-graph`/`test:change-graph` from the root scripts.
  One implementation, or the two drift.
- [x] 3.5 Rewire `scripts/change-graph.mjs` to read through core. It is
  the one caller that can afford a build, because it renders rather than
  gates.

## 4. Prove the move changed nothing

  Rewired, and then moved. Plain `node` cannot import core's TypeScript
  (`ERR_UNKNOWN_FILE_EXTENSION`), and `--experimental-strip-types` fails
  on this repository's `./protocol.js` import convention, which resolves
  to a `.ts` file. So the renderer went where this change's own reasoning
  points: `packages/cli/src/change-graph-render.ts`, which compiles with
  the package that already wraps core for terminal use.

  That reverses `change-dependency-graph`'s deviation, and the reversal is
  the point rather than an embarrassment: it put the renderer in
  `scripts/` to avoid a second copy of the parser, and this change removes
  that reason by centralising the reader. `npm run graph:changes` runs it
  through `tsx` — no build, which is what the proposal promised.
- [x] 4.1 Run the old script and the new test against this repository
  before deleting the script, and confirm both report the same thing on
  the same six backfilled edges.
  Recorded 2026-09-06 before deleting the script, by running it over this
  repository and over each failure case:

  | Case | Old script |
  | --- | --- |
  | this repository | 0 violations |
  | edge to nothing | 1 |
  | cycle | 1 |
  | glued key | 1 |
  | unreadable shape | 1 |
  | no relation at all | 0 |

  `change-graph.test.ts` asserts the same six outcomes, so the port kept
  what the script caught.
- [x] 4.2 Reintroduce, temporarily, each failure the old check caught —
...
- [x] 4.3 Confirm the gate runs on a checkout where nothing is built:
  `npm ci && npm run test` with no build step, or the equivalent, so the
  claim in the proposal is measured rather than assumed.

## 5. Verification

  Confirmed: `npm run test` runs the gate with nothing built. The test
  lives in `packages/core` and vitest executes TypeScript directly — the
  reason the check is a test rather than a script.
- [x] 5.1 `openspec change validate --strict change-graph-in-core`.
...
- [x] 5.3 Version bump via `npx changeset`: `@openspec-ui/core` gains
  exported API. Unlike `change-dependency-graph`, which touched nothing
  published, this one does.
- [x] 5.4 Declare `blocked_by` on the three changes planned after this
