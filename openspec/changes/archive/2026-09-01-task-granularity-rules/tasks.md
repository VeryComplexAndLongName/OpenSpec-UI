...
- [x] 1.1 `openspec/config.yaml`, `rules.tasks`: append a rule stating
...
- [x] 1.2 `openspec/config.yaml`, `rules.tasks`: append a rule requiring
...
- [x] 1.3 `openspec/config.yaml`, `rules.tasks`: append a rule stating
  that when a plausible-looking generalization of a task would be wrong,
  the task must state the prohibition explicitly.
- [x] 1.4 `openspec/config.yaml`, `rules.tasks`: append a rule stating
...
- [x] 1.5 `openspec/config.yaml`, `rules.tasks`: append a rule stating
...
- [x] 1.6 `openspec/config.yaml`, `rules.tasks`: append a rule stating
  that when a change threads a new value through layers, the tasks must
  name the complete path it travels (origin through every intermediate
  layer to the point of use) and require a check at each junction — not
  only at the two ends. State the reason: a task phrased as "widen the
  type here" is most cheaply satisfied by discarding the new value at
  that layer, which type-checks and passes every existing test.

## 2. Verification

- [x] 2.1 `openspec change validate --strict task-granularity-rules`.
- [x] 2.2 `npx openspec instructions` (or the equivalent used by the
...
- [x] 2.3 `openspec/config.yaml` remains valid YAML and every other
  ...
  only.
- [x] 2.4 No changeset needed (process/tooling only, no `packages/*`
