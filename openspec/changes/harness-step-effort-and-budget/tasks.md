...
- [x] 1.2 Same file: `HarnessStepAgent`'s object form gains
...
- [x] 1.3 Same file: `normalizeStepAgent` carries both through. The bare
  string form keeps working and yields neither field.
- [x] 1.4 Same file: export, per agent id, which effort values that agent
...
- [x] 2.1 `packages/core/src/harness-config.ts`: a stage entry whose
...
- [x] 2.2 Same file: a stage entry setting `effort` for an agent that has
...
- [x] 2.3 Same file: a stage entry setting a budget field its agent does
  not honour is rejected the same way — `maxCostUsd` on `copilot-cli`, or
  `maxAiCredits` on `claude-cli`.
- [x] 2.4 Same file: both fields resolve through the existing global +
...
- [x] 2.5 `harness-config.test.ts`: a global-only value, a per-change-only
...
- [x] 3.1 `packages/core/src/agents/claude.ts`: append `--effort <level>`
...
- [x] 3.2 `packages/core/src/agents/copilot.ts`: append `--effort <level>`
...
- [x] 3.3 `packages/core/src/agents/codex.ts`: append
...
- [x] 3.4 `packages/core/src/agents/gemini.ts`: render nothing for either
...
- [x] 3.5 `packages/core/src/agents/claude.ts`: `--max-budget-usd`
...
- [x] 3.6 No adapter changes any argv it already produced. A stage entry
  without the new fields must yield a byte-identical command line.

## 4. Allowlist

- [x] 4.1 `packages/core/src/default-runners.ts`: replace
...
- [x] 4.2 Same file: each optional value has its own validator —
...
- [x] 4.3 Same file: for `codex-cli`, match the whole pair including the
  key — `-c` followed by `model_reasoning_effort="<accepted level>"` — and
  nothing else beginning with `-c`.
- [x] 4.4 Same file: keep the matcher **closed**. Any argv that is not the
  expected prefix plus a permitted subset of these pairs, in order, is
  still refused.
- [x] 4.5 `default-runners.test.ts`: today's argv still matches; each new
...
- [x] 5.1 `packages/webui/src/components/HarnessSettingsView.tsx`: both
  fields per stage, offering only the values that stage's agent accepts.
- [x] 5.2 `packages/extension`: the same, wherever it presents the same
  settings.
- [x] 5.3 Neither surface may offer a value the validator would reject —
  the picker and the validator read the table from task 1.4.

## 6. Verification

- [x] 6.1 `openspec change validate --strict harness-step-effort-and-budget`.
...
- [x] 6.4 A stage entry with neither field produces a byte-identical argv
