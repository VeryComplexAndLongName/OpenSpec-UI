...
- [x] 1.1 `packages/core/src/protocol.ts`: add `"verify"` to `CommandKind`
  ...
  an event.
- [x] 1.2 `packages/core/src/agents/shared.ts`,
...
- [x] 1.3 Same function: reword `case "review"` so it describes reviewing
...
- [x] 1.4 `packages/core/src/protocol.test.ts`: `COMMAND_KINDS` contains
  `"verify"`, and every previously present kind is still present with its
  existing behavior.

## 2. Stage wiring

- [x] 2.1 `packages/core/src/harness-stage.ts`: add `"verify"` to
...
- [x] 2.2 `packages/core/src/harness-chain-runner.ts`: `CHAIN_STAGES`
  becomes `["propose", "review", "apply", "verify", "archive"]`.
- [x] 2.3 Same file: `CHAIN_STAGE_COMMAND` gains `verify: "verify"`. Its
  type currently covers only `"propose" | "review" | "apply"` — widen it to
  include `"verify"`, and do **not** widen it to `"archive"`, which is not
  an agent run.
- [x] 2.4 Same file: `determineStartStage()` must return `"verify"` for a
...
- [x] 2.5 `packages/core/src/harness-config.ts`: `stepAgents` accepts a
...
- [x] 3.1 `packages/core/src/security.ts`: `AgentPromptContextOptions`
...
- [x] 3.2 Same file: when that field is present, `prepareAgentContext()`
...
- [x] 3.3 Same file: do **not** call `GitWrapper.diff()` from here or
...
- [x] 3.4 Same file: when the delta exceeds the section's size budget,
...
- [x] 3.5 `packages/core/src/harness-chain-runner.ts`: the `verify` stage
...
- [x] 4.1 `packages/extension/src/describe-event.ts`: handle the `verify`
  command kind in its exhaustive switch, the same widening every previous
  additive protocol change performed.
- [x] 4.2 `packages/webui/src/components/AiPanel.tsx`, `describeEvent()`:
  the same.
- [x] 4.3 `packages/extension` and `packages/webui`: wherever a stage list
  is presented to a user, `verify` appears in chain order. No new business
  logic in either package, per ADR 0001.

- [x] 4.4 `packages/extension/src/workbench-chat-prompt.ts`, `STAGE_VERB`:
  add a `verify` entry. Without one the map's `?? "Work on"` fallback
  applies, so a `verify` stage dispatched with `dispatch: "vscode-chat"`
  asks the chat to "Work on" the change rather than to check it. Found in
  review 2026-09-01; this file was missing from section 4's original list.
- [x] 4.5 Same file, same map: `review` currently reads "Review the
  implementation of". That is the identical false statement task 1.3
  corrected in `commandInstruction("review")` — the `review` stage runs
  before `apply`, so there is no implementation yet. Reword it to describe
  reviewing the change's proposal. Do **not** leave the two surfaces
  disagreeing: they describe the same stage to the same reader.

## 5. Tests

- [x] 5.1 `harness-chain-runner.test.ts`: a chain runs `verify` between
  `apply` and `archive`, using `stepAgents.verify`'s agent when set and the
  default agent when unset.
- [x] 5.2 `harness-chain-runner.test.ts`: `determineStartStage()` returns
  `"verify"` for a change with all tasks checked and no archive, and still
  returns `"apply"` when task counts cannot be read.
- [x] 5.3 `security.test.ts`: with a delta supplied, the `verify` prompt
  ...
  byte-identical to today's.
- [x] 5.4 `security.test.ts`: an oversized delta produces a truncated
  section that states the number of omitted files, and never an empty or
  absent section.
- [x] 5.5 `harness-config.test.ts`: `stepAgents.verify` resolves through
  the same global/per-change merge as the other entries; an unset entry
  behaves exactly as an unset `review` does today.
- [x] 5.6 A test asserting the chain still stops before `archive` when
