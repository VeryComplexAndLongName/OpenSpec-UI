...
- [x] 1.1 Define the `HarnessConfig` type and Zod (or equivalent)
...
- [x] 1.2 `readGlobalHarnessConfig(workspaceRoot)`: reads `openspec/
  agent-harness.json`; returns a documented all-`assisted`/
  `human-required` default when the file doesn't exist (never throws for
  a missing file).
- [x] 1.3 `readChangeHarnessConfig(workspaceRoot, changeName)`: reads
  `openspec/changes/<changeName>/harness.json` if present; returns
  `undefined` if absent.
- [x] 1.4 `resolveHarnessConfig(workspaceRoot, changeName?)`: deep-merges
...
- [x] 1.5 `writeGlobalHarnessConfig`/`writeChangeHarnessConfig`: validate
  before writing; never write a structurally invalid file.
- [x] 1.6 Unit tests: default-when-missing, partial per-key override
...
- [x] 3.2 No change if no harness config exists for the workspace/change
  (falls back to today's existing last-used-agent behavior exactly).
- [x] 3.3 Tests confirming pre-fill only changes the *default* selection,
  never blocks or overrides an explicit user pick before Run.

## 4. Harness Settings GUI (both delivery targets)

- [x] 4.1 Standalone webui: new settings section/tab to view/edit
...
- [x] 4.2 Standalone webui: per-change "Configure Harness for this
...
- [x] 4.3 VS Code extension: equivalent tree entry ("Harness Settings")
...
- [x] 4.4 Real Extension Host smoke test (per this project's established
...
- [x] 5.1 `openspec/specs/agentic-harness/spec.md` delta (this change's
  `specs/agentic-harness/spec.md`) — new capability, `ADDED
  Requirements`.
- [x] 5.2 Do not begin implementation (tasks 1–4) until
...
- [x] 5.3 `openspec change validate --strict agentic-harness`.
- [x] 5.4 typecheck/lint/test for `core`, `webui`, `extension`; real
  Extension Host smoke test (see 4.4) before this change may be archived.
- [x] 5.5 Version bump via `npx changeset` (per this repo's adopted
