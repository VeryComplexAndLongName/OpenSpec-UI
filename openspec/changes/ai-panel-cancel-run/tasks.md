...
- [ ] 1.1 `packages/webui/src/components/AiPanel.tsx`: add a helper that
...
- [ ] 1.2 Same file: render a Cancel button in the existing
...
- [ ] 1.3 Same file: give the button `data-testid="cancel-run-button"`,
  matching the naming of `run-button` and the chain panel's
  `cancel-chain-button`.
- [ ] 1.4 Same file: do **not** re-check `isRunning` inside the click
...
- [ ] 2.1 `packages/extension/package.json`: change
...
- [ ] 2.2 Do **not** change what that command does, its `when` clause, or
...
- [ ] 3.1 `packages/webui/src/components/AiPanel.test.tsx`: while a run is
  in flight, the Cancel button is present; before any run and after a
  terminal event, it is absent.
- [ ] 3.2 Same file: clicking Cancel sends exactly one command with
...
- [ ] 3.3 Same file: the cancel command carries the same `changeDir` the
  run was started with, including the `list` case where it is the changes
  root rather than a change directory.
- [ ] 3.4 Same file: a run that reaches a terminal event and is then
  cancelled — by a click that raced the event — sends the command without
  throwing, and the panel still shows the terminal state.
- [ ] 3.5 `packages/extension`: whatever test asserts the contributed
  command titles is updated for the new title, and no test asserting the
  command **id** changes.

## 4. Verification

- [ ] 4.1 `openspec change validate --strict ai-panel-cancel-run`.
- [ ] 4.2 `npm run typecheck` and `npm run test` — green across all four
...
- [ ] 4.3 `git diff packages/core/` is **empty**. Cancellation behavior
...
- [ ] 4.4 Version bump via `npx changeset` (`@openspec-ui/webui` and
  `openspec-ui-vscode`).
