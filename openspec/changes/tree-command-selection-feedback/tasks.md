...
- [ ] 1.1 `packages/extension/src/commands.ts`, next to the existing
  ...
  the tree first.\`)`).
- [ ] 1.2 Update the three pre-existing ad hoc
  `showErrorMessage("OpenSpec UI: open a folder or workspace first.")`
...
- [ ] 2.1 `configureHarnessForChange` (line ~487) — "change"
- [ ] 2.2 `runWithHarness` (line ~507) — "change"
- [ ] 2.3 `validateSelectedChange` (line ~663) — "change"
- [ ] 2.4 `showChangeTimeline` (line ~683) — "change"
- [ ] 2.5 `archiveChange` (line ~696) — "change" (the change that
...
- [ ] 2.6 `unarchiveChange` (line ~719) — "change"
- [ ] 2.7 `copyTasksAsTemplate` (line ~741) — "change"
- [ ] 2.8 `customizeTemplate` (line ~765) — "template"
- [ ] 2.9 `insertTemplateIntoChange` (line ~791) — "template"
- [ ] 2.10 `deleteProjectTemplate` (line ~841) — "template"
- [ ] 2.11 `deleteChange` (line ~862) — "change"
- [ ] 2.12 `revealTask` (line ~884) — "task" (currently has no
  `workspaceRoot` check at all — only add the "no item" branch, do not
  invent a workspace-root requirement that was never there)
- [ ] 2.13 `deleteTask` (line ~897) — "task"
- [ ] 2.14 `startImplementation` (line ~921) — "change"
- [ ] 2.15 `rollbackChange` (line ~983) — "change" (same note as
  `revealTask`: no pre-existing `workspaceRoot` check, don't add one)

## 3. Tests

- [ ] 3.1 `commands.test.ts`: for each of the 15 commands, one new test
...
- [ ] 3.2 `commands.test.ts`: one test confirming `warnNoWorkspace()`'s
...
- [ ] 3.3 Existing happy-path tests for all 15 commands still pass
...
- [ ] 4.2 typecheck/lint/test for `extension`.
...
- [ ] 4.4 `openspec/specs/vscode-extension/spec.md` delta — new
  requirement, scenarios matching tasks 3.1.
- [ ] 4.5 Version bump via `npx changeset` (`openspec-ui-vscode`, patch —
