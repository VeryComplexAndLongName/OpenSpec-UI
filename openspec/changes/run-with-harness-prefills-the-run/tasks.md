...
- [x] 1.1 `packages/webui/src/components/AiPanel.tsx`: seed the selected
...
- [x] 1.2 Same file: accept an optional initial command kind from the
  host, defaulting to today's `list`. A host that names no change has
  nothing to run and `list` remains right for it.
- [x] 1.3 Do **not** touch the agent pre-selection effect. It already
...
- [x] 4.1 `AiPanel.test.tsx`: given a `changeDir`, that change is
  selected before any list has loaded, and stays selected once the list
  arrives containing it.
- [x] 4.2 Same: given the seeded `implement` kind and a `stepAgents`
  entry for `apply`, the agent picker shows that agent — asserting the
  consequence in task 1.3, not a re-implementation of it.
- [x] 4.3 Same: with no seed, the panel behaves exactly as before —
  `list`, no change, default agent. This is the path every other entry
  point uses.
- [x] 4.4 `packages/extension`: `runWithHarness` reveals the panel with
  the seed; another reveal does not.

## 5. Verification

- [x] 5.1 `openspec change validate --strict run-with-harness-prefills-the-run`.
