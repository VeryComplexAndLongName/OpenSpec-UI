...
- [ ] 1.1 `packages/core/src/harness-chain-runner.test.ts`, "confirming a
...
- [ ] 1.2 Do **not** change any assertion in that test. It asserts the
...
- [ ] 2.1 The file has seven `vi.waitFor` call sites. Give the same
  ...
  ones unreadable.
- [ ] 2.2 Check the rest of `packages/core`'s tests for `vi.waitFor` on
...
- [ ] 2.3 Do **not** raise vitest's global `testTimeout` or a global
...
- [ ] 3.1 `openspec change validate --strict load-sensitive-test-timeouts`.
- [ ] 3.2 `packages/core`'s suite passes **inside a full `npm run test`**,
  not only alone. That distinction is the whole change: the test already
  passed alone before it.
- [ ] 3.3 Run the full suite with a deliberate co-load, the condition
  under which this failed on 2026-09-02, and record the result.
- [ ] 3.4 `git diff` shows no changed `expect` and no non-test file.
- [ ] 3.5 No changeset — test-only, matching
