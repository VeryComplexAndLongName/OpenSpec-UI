...
- [ ] 1.1 `packages/core/src/workbench-run-journal.ts`: `WorkbenchRunJournalDocument`'s
...
- [ ] 1.2 Same file: `WORKBENCH_RUN_JOURNAL_VERSION` becomes `2`. Do
...
- [ ] 1.3 Same file: a session is stored at
...
- [ ] 2.2 Same method: a session whose file already exists is **not**
  rewritten. A finalized checkpoint never changes; rewriting it is the
  cost this change exists to remove.
- [ ] 2.3 Same method: keep the temporary-file-then-`rename` atomic
  ...
  slow one.
- [ ] 2.4 Same method: writing a process state change must not read or
...
- [ ] 3.1 `packages/core/src/workbench-run-journal.ts`, `load()`: for a
  ...
  changes.
- [ ] 3.2 Same method: a referenced session whose file is missing or
...
- [ ] 3.3 Same method: checkpoint files that no journal entry references
  are deleted during load. The journal is the authority on what exists.
- [ ] 3.4 Same method: a version-1 journal is migrated — each embedded
...
- [ ] 3.5 Same method: a journal whose version is neither 1 nor 2 still
  reports `unsupported-journal-version`, unchanged.

## 4. Retention

- [ ] 4.1 `packages/core/src/workbench-run-journal.ts`: when a process is
...
- [ ] 4.2 Do **not** change the retention policy itself — which processes
  are pruned, or when. Only what is deleted alongside them.

## 5. Tests

- [ ] 5.1 `packages/core/src/workbench-run-journal.test.ts`: a round trip
  ...
  change.
- [ ] 5.2 Same file: **the size assertion.** After saving a journal with a
  ...
  present.
- [ ] 5.3 Same file: a version-1 journal containing embedded sessions
  migrates to version 2, its sessions become files, and the loaded data
  matches what version 1 held.
- [ ] 5.4 Same file: a referenced session whose file was deleted loads
  successfully, reporting that session without a checkpoint, and the
  process history is complete.
- [ ] 5.5 Same file: a checkpoint file that no journal entry references is
  removed by load.
- [ ] 5.6 Same file: pruning a process deletes its checkpoint file.
- [ ] 5.7 Same file: an unsupported journal version and an unsupported
  checkpoint version each still produce their existing error codes.

## 6. Verification

- [ ] 6.1 `openspec change validate --strict checkpoint-storage-split`.
- [ ] 6.2 `npm run typecheck` and `npm run test` — green across all four
  ...
  here.
- [ ] 6.3 `git diff packages/core/src/checkpoint.ts` is **empty**. What a
