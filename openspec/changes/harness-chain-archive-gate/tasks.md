...
- [ ] 1.1 `packages/core/src/openspec.ts`, `normalizeStatusResult`
...
- [ ] 1.2 `packages/core/src/openspec.ts`: make `progress` optional on
...
- [ ] 1.3 Fix every consumer the optional type breaks. The known one is
...
- [ ] 2.1 `packages/core/src/harness-chain-runner.ts`: add a helper that
  ...
  cannot be read.
- [ ] 2.2 `packages/core/src/harness-chain-runner.ts`,
...
- [ ] 2.3 Same function: when the helper returns `undefined`, return
  `apply`, never `archive` — see design.md, "The chain counts tasks
  itself, and fails safe".
- [ ] 2.4 Same function: do **not** read `status.progress` here any more.

## 3. Gate the archive stage

- [ ] 3.1 `packages/core/src/harness-chain-runner.ts`, in the
...
- [ ] 3.2 Same file: the failure `reason` must name the change and the
...
- [ ] 3.3 Same file: do **not** touch the `apply` stage and do **not**
  compare task counts before and after it — design.md explains why that
  check produces false positives.

## 4. Tests

- [ ] 4.1 `openspec.test.ts`: a CLI result without `progress` yields a
  result whose `progress` is absent — not a synthesized zero-remaining
  value.
- [ ] 4.2 `openspec.test.ts`: a CLI result that does carry `progress`
  passes it through unchanged.
- [ ] 4.3 `harness-chain-runner.test.ts`: **the incident, as a test** —
  a change whose artifacts all exist and whose `tasks.md` has every task
  unchecked starts at `apply`, not `archive`.
- [ ] 4.4 `harness-chain-runner.test.ts`: a change with all tasks checked
  starts at `archive`, as before.
- [ ] 4.5 `harness-chain-runner.test.ts`: an unreadable `tasks.md` starts
  at `apply`.
- [ ] 4.6 `harness-chain-runner.test.ts`: reaching `archive` with
  unchecked tasks emits `failed`, and `archiveChange` is **not** called.
- [ ] 4.7 `harness-chain-runner.test.ts`: reaching `archive` with every
  task checked archives as before.
- [ ] 4.8 `harness-chain-runner.test.ts`: the failure reason contains the
  change name and the remaining count.

## 5. Verification

- [ ] 5.1 `openspec change validate --strict harness-chain-archive-gate`.
- [ ] 5.2 `npm run typecheck`/`lint`/`test` for `@openspec-ui/core` and
...
- [ ] 5.3 `openspec/specs/agentic-harness/spec.md` and
