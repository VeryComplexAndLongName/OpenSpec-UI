...
- [x] 1.1 Root `package.json`: `changeset:version` runs
  ...
  CI does.
- [x] 1.2 `.github/workflows/quality.yml`: point the release action at
...
- [x] 1.3 Correct today's stale entry (`packages/cli`, lockfile 0.1.2
  against package.json 0.2.0) in the same change.

## 2. Establish the step is safe

- [x] 2.1 Run `npm install --package-lock-only` against this repository
...
- [x] 2.2 Say in the proposal what was measured, so the next person can
  re-check it rather than trust it — npm's resolution behaviour is not a
  fixed property of the tool.

## 3. Verification

- [x] 3.1 `openspec change validate --strict lockfile-follows-the-release`.
- [x] 3.2 Assert the lockfile and the five `package.json` files agree,
  by reading both rather than by eye.
- [x] 3.3 `npm ci` still succeeds against the corrected lockfile — the
