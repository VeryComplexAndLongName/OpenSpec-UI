...
- [ ] 1.1 `packages/core/src/harness-step-agent.ts`: add an agent id for
...
- [ ] 1.2 Same file: `normalizeStepAgent` stops returning `dispatch`.
  ...
  removes.
- [ ] 1.3 `packages/extension/src/webview/ai-panel.ts` and any other
...
- [ ] 1.4 `packages/core/src/default-runners.ts`: the new id needs no
...
- [ ] 2.1 `packages/core/src/harness-config.ts`,
  ...
  accepted".
- [ ] 2.2 Same function: keep every existing per-agent check exactly as
...
- [ ] 2.3 Same function: the assisted-only rule that `dispatch:
  "vscode-chat"` carried moves to the new agent id, unchanged. A chain
  still cannot use it.

## 3. Unknown keys are errors

- [ ] 3.1 `packages/core/src/harness-config.ts`: a stage entry carrying a
...
- [ ] 3.2 Do **not** warn instead of refusing. A warning about a known
...
- [ ] 3.3 Same treatment for unknown keys inside `budget`. A typo there
  is as invisible as one a level up.
- [ ] 3.4 Do **not** extend unknown-key rejection to `autonomyLevel`,
  `reviewGate` or `checkpoints` in this change. They are out of scope,
  and widening it here would mix a targeted fix with a sweep.

## 4. Migration

- [ ] 4.1 `packages/core/src/harness-config.ts`: a configuration using
...
- [ ] 4.2 Same: `dispatch: "cli"` is dropped silently — it was the
  default, so there is nothing to tell anyone.
- [ ] 4.3 A configuration that set `model`, `effort` or `budget`
...
- [ ] 4.4 `openspec/agent-harness.json`: leave this repository's own file
...
- [ ] 5.1 `packages/webui/src/components/HarnessSettingsView.tsx`: the
...
- [ ] 5.2 `packages/extension`: the same wherever it presents these
  settings.

## 6. Tests

- [ ] 6.1 `harness-config.test.ts`: the chat agent with a `model`, with
  an `effort`, and with a `budget` each produce their named error.
- [ ] 6.2 Same file: an unknown top-level key and an unknown key inside
  `budget` each produce an error naming the key and the accepted set.
- [ ] 6.3 Same file: a configuration using `dispatch: "vscode-chat"`
  loads, is mapped, and reports; one using `dispatch: "cli"` loads
  silently; one combining `dispatch: "vscode-chat"` with a `model` fails.
- [ ] 6.4 Same file: every existing valid configuration in this
...
- [ ] 6.5 Same file: the chat agent under `semi-autonomous` or
  `autonomous` is still refused.

## 7. Verification

- [ ] 7.1 `openspec change validate --strict harness-config-strictness`.
- [ ] 7.2 `npm run typecheck`, `npm run lint` and `npm run test` — green
  across all four workspaces.
- [ ] 7.3 `git diff packages/core/src/agents/` is **empty** apart from
  `registry.ts`'s comment and the new id. This change is about naming and
  validation, not about how any agent runs.
- [ ] 7.4 Version bump via `npx changeset` (`@openspec-ui/core` minor,
